...
func place_plant() -> void:
	...
	var seed_item := GlobalData.plant_selected.seed_item
	if seed_item == null or GlobalData.get_seed_count(seed_item) <= 0:
		return

	var tile: Vector2i = ground_area.get_tile_from_global(player.global_position)
	spread_seeds(tile, seed_item)


func spread_seeds(center_tile: Vector2i, seed_item: SeedResource) -> void:
	for x in range(-1, 2):
		for y in range(-1, 2):
			if GlobalData.get_seed_count(seed_item) <= 0:
				return
			...
			if ground_area.can_plant_first_seed(target_tile):
				...
				if existing != null and existing.has_method("try_crossbreed_with"):
					if existing.try_crossbreed_with(GlobalData.plant_selected):
						GlobalData.remove_seed(seed_item)
